...
class Pareto():
    ...
    def train(self,max_episodes,max_steps,log,plot_paretofront):
        ...
        while numberOfEpisodes  < max_episodes:
            ...
            numberOfEpisodes+=1
            print("episode = " + str(numberOfEpisodes) +  " | rewards = [" + str(acumulatedRewards[0]) + "," + str(acumulatedRewards[1]) + "]")
    ...
    def step(self,s):
        ...
        self.current_trajectory.append((s, action))
...
def hv_plot(ref, q_set):
    # Create a new figure and axis
    fig, ax = plt.subplots()

    # List of distinct colors for the rectangles
    colors = ['lightpink', 'lightblue', 'lightgreen', 'lightyellow']

    # Plot the reference point with a larger size (e.g., s=200)
    ax.scatter(*ref, color='blue', label='Ref Point (0,0)', s=300, zorder=5)  # zorder=5 to make it appear on top

    # Sort q_set by area size (width * height) in ascending order
    areas = [point[0][0] * point[0][1] for point in q_set]
    sorted_indices = np.argsort(areas)
    q_set = q_set[sorted_indices]

    # Plot the points and areas
    patches_list = []  # To store the patches for the legend
    for i, point in enumerate(q_set):
        point = point[0]
        color = colors[i % len(colors)]
        ax.scatter(*point, color=color, s=100)
        rect = patches.Rectangle(ref, point[0], point[1], facecolor=color, alpha=0.3, zorder=1)  # zorder=1 to make it appear behind
        ax.add_patch(rect)
        patches_list.append(patches.Patch(color=color, label=f'Point {tuple(point)}', alpha=0.2))

    # Set the axis limits
    ax.set_xlim(0, np.max(q_set) + 1)
    ax.set_ylim(0, np.max(q_set) + 1)
    ...
    legend1 = plt.legend(handles=[patches.Patch(color='blue', label='Ref Point (0,0)')], loc='upper left')
...
if __name__ == '__main__':
    ...
    q_set = np.array([[[1, 4]], [[2., 3]],[[3., 2]]])
    q_set = q_set.reshape(3, 1, 2)
    print(q_set.shape)
    nA = 2
    ref = np.array([0, 0])
    volumes = compute_hypervolume(q_set, nA, ref)
    print(volumes)
    ...
    env = DeepSeaTreasure()
    #env = deepst.DeepSeaTreasure(dst_map =deepst.TEST_MAP4x4)
    numberOfStates = 121
    ...
    #metrics
    #metrics.plotGraph()
    #met.plot_pareto_frontier()
    trajectories = [agent.trajectories[900]]
    from testing_functions import plot_grid
    plot_grid(grid_size = env.size, trajectories = trajectories , map_values = deepst.TEST_MAP4x4)
